...
# from api_spec_converter.utils import parse_json, parse_yaml


# TODO: Find or implement a robust Python equivalent for OpenAPI 3 to Swagger 2 conversion.
# This is a placeholder for the actual conversion logic.
def openapi3_to_swagger2_converter(
    openapi3_instance: "OpenAPI3Format", passthrough_options: dict = None
) -> dict:
    """
    ...
    """
    # print(
    #     "Warning: openapi_3 to swagger_2 conversion is a basic placeholder and not fully implemented."
    # )
    if not openapi3_instance.spec:
        return {
            "swagger": "2.0",
            "info": {"title": "Empty Spec", "version": "1.0.0"},
            "paths": {},
        }
    ...
    if servers and isinstance(servers, list) and servers[0].get("url"):
        ...
        if converted_spec["host"] and base_path == "/":
            converted_spec["basePath"] = (
                None  # Or prefer to keep it as "/"? Swagger tools vary.
            )
        else:
    ...
    if converted_spec["host"] is None:
        converted_spec.pop("host")
    if converted_spec["basePath"] is None:
        converted_spec.pop("basePath")
            ...
            if isinstance(path_item, dict):
                for op_name, operation in path_item.items():
                    if isinstance(operation, dict) and "requestBody" in operation:
                        ...
                        if "content" in request_body:
                            ...
                            if (
                                first_content_type
                                and "schema"
                                in request_body["content"][first_content_type]
                            ):
                                body_param = {
                                ...
                                if request_body.get("description"):
                                    body_param["description"] = request_body[
                                        "description"
                                    ]
                    ...
                    if isinstance(operation, dict) and "responses" in operation:
                        ...
                        for resp_code, response_obj in operation.get(
                            "responses", {}
                        ).items():
                            if (
                                ...
                                first_content_key = next(
                                    iter(response_obj["content"]), None
                                )
                                if first_content_key:
                                    # Use schema from the first content type found
                                    response_obj["schema"] = response_obj["content"][
                                        first_content_key
                                    ].get("schema")
                                ...
                                response_obj.pop(
                                    "content"
                                )  # Remove OpenAPI 3 content structure
                        if (
                            global_produces and "produces" not in converted_spec
                        ):  # Set global produces if derived
                            converted_spec["produces"] = list(
                                set(global_produces)
                            )  # Unique list
...
class OpenAPI3Format(BaseFormat):
    ...
    def supported_versions(self) -> list[str]:
        ...
        return [
            ...
            "3.0.2",
            "3.0.3",
        ]  # Add "3.1.0" etc. as support is added
    ...
    def get_format_version(self) -> str:
        ...
        raise ValueError(
            "OpenAPI 3.x version field ('openapi') not found in specification."
        )
    ...
    def check_format(self, spec_data: dict) -> bool:
        ...
        return (
            ...
            and "openapi" in spec_data
            and isinstance(spec_data["openapi"], str)
            and (
                spec_data["openapi"].startswith("3.0.")
                or spec_data["openapi"].startswith("3.1.")
            )
        )  # Future proof for 3.1
    ...
    def fix_spec(self):
        ...
        if (
            "info" in self.spec
            and isinstance(self.spec["info"], dict)
            and "version" in self.spec["info"]
        ):
    ...
    def fill_missing(self, dummy_data: dict = None):
        ...
        """
        default_dummy_data = {
            "openapi": self.supported_versions[0]
            if self.supported_versions
            else "3.0.0",  # Default to first supported
            "info": {"title": "< An API title here >", "version": "1.0.0"},
            "paths": {},
            "servers": [{"url": "/"}],
        }
        ...
        if (
            ...
            or not self.spec["servers"]
            or not (
                isinstance(self.spec["servers"], list)
                and all(isinstance(s, dict) for s in self.spec["servers"])
            )
        ):
    ...
    def validate(self) -> dict:
        ...
        # print(
        #     "Warning: OpenAPI 3.0.x validation is a placeholder and not fully implemented."
        # )
    ...
    def list_sub_resources(self) -> dict:
        ...
        Returns:
            dict: A map of reference identifiers to external URIs. Example: {"#/components/schemas/ExternalModel": "external_models.yaml#/ModelA"}
        """
